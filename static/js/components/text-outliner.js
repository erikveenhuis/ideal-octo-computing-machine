 ...
 *
 * Font selection rules (mirrors the styles emitted by
 * [static/js/gpx-app.js](static/js/gpx-app.js) and the marker text in
 * [static/js/components/feature-converter.js](static/js/components/feature-converter.js)):
 *   - font-weight: bold / 600+ / "bolder" -> DIN Pro Bold (dinpro_bold.otf)
 *   - everything else                     -> DIN Pro Regular (dinpro.otf)
 *
 ...
 */
class TextOutliner {
    /** Cached parsed opentype fonts keyed by ``regular`` / ``bold``.
     * Class-level so a single export run that calls outlineSvgTextNodes
     * multiple times only fetches each font once. */
    ...
    /**
     * Load both DIN Pro variants and return ``{regular, bold}`` parsed
     ...
     *     dedicated attributes, or inherited from the <text>)
    ...
    static _outlineOneText(textEl, fonts) {
        ...
        if (tspans.length > 0) {
            // Anchor every tspan against the <text>'s text-anchor.
            for (const tspan of tspans) {
                if (this._appendOutlinedRun(wrapper, tspan, textDefaults, textAnchor, fonts)) {
                    anyEmitted = true;
                }
            }
        } else {
            // Single-line <text>. Use its own x/y/dx/dy and inline paint.
            if (this._appendOutlinedRun(wrapper, textEl, textDefaults, textAnchor, fonts)) {
                anyEmitted = true;
            }
        }
     ...
     */
    static _appendOutlinedRun(wrapper, runEl, parentDefaults, parentAnchor, fonts) {
        const text = (runEl.textContent || '').trim();
        if (!text) return false;

        // Resolve paint + font properties for THIS run, falling back to
        // the parent <text>'s defaults.
        const paint = { ...parentDefaults, ...this._readPaintAttrs(runEl) };
        const fontSize = paint.fontSize > 0 ? paint.fontSize : 12;
        ...
        // text in a surrounding circle.
        const x = this._readNumericAttr(runEl, 'x', 0);
        const y = this._readNumericAttr(runEl, 'y', 0);
        ...
        const advance = this._safeAdvanceWidth(font, text, fontSize);
        ...
        const renderY = y + dy;
        const otPath = font.getPath(text, renderX, renderY, fontSize);
        ...
        if (paint.fillOpacity !== null && paint.fillOpacity < 1) {
    ...
    /** Compute the advance width of a string in the given font + size.
     * Falls back to 0 if opentype reports a non-finite value (e.g. a
     * glyph that's missing from the font). */
    static _safeAdvanceWidth(font, text, fontSize) {
        try {
    ...
    static _pickFontVariant(weight, fonts) {
        if (typeof weight === 'number') {
            return weight >= 600 ? fonts.bold : fonts.regular;
        }
